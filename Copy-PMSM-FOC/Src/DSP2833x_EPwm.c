...
void  QEP_Init(void)
{
    ...
    EQep1Regs.QEPCTL.all=0x820a;//QEPI发生时复位位置计数器、QEPI上升沿初始化计数器、QEP计数使能、使能计时器
    EQep1Regs.QPOSCNT=0;//位置计数器清零
    EQep1Regs.QPOSINIT=0;//位置寄存器初始化寄存器
    TotalPulse=_IQmpy(_IQ(4),LineEncoder);//一圈的脉冲信号数=4×2500
    EQep1Regs.QPOSMAX= (Uint16)(TotalPulse>>15);//最大位置寄存器的值   1 0000（Q0）
    EQep1Regs.QCAPCTL.all=0x8075;//使能捕获单元、单位位置32分频、CAP捕获128分频
    EDIS;
...
EPwm4Regs.TBPRD= 375000/100; // Period = 1600 TBCLKcounts Tpwm=TBPRD*Ttbclk,100表示20k
...
EPwm4Regs.TBCTL.bit.CTRMODE=0;//增计数
...
EPwm4Regs.DBCTL.bit.OUT_MODE = DB_DISABLE; // 不经过死区
...
//pwm dac 说明， pwm 占空比 3750 对应的是 5v  其它值的输出 就懂了吧，只能输出正电压的哦。

}

void DAC1_out(_iq data)//data是标么值
{
    Uint16 temp=0;
    if(data<0)
    {
...
void DAC2_out(_iq data)//data是标么值
{
    Uint16 temp=0;
    if(data<0)
    {
...
{
    EALLOW;
    SysCtrlRegs.PCLKCR0.bit.TBCLKSYNC = 0;   //禁止PWM模块时钟同步（时钟设定——外设时钟）
    EDIS;
    EALLOW;
...
EPwm1Regs.TBCTL.bit.PHSDIR=1;//相位方向位：同步信号来临时，向上计数
EPwm1Regs.TBCTL.bit.CLKDIV=0;
EPwm1Regs.TBCTL.bit.HSPCLKDIV=1;
EPwm1Regs.TBCTL.bit.SYNCOSEL=0x3; //同步信号输出：禁止同步信号输出
EPwm1Regs.TBCTL.bit.PRDLD=0;//影子模式
EPwm1Regs.TBCTL.bit.PHSEN=0;//禁止装载
EPwm1Regs.TBCTL.bit.CTRMODE=2;//增减计数

EPwm1Regs.CMPCTL.all=0x0000;//在计数值为零时cmpb影子装载、cmpa影子装载；

EPwm1Regs.AQCTLA.bit.CAU=AQ_CLEAR;//ipm低电平有效 1a
EPwm1Regs.AQCTLA.bit.CAD=AQ_SET;

EPwm1Regs.DBCTL.bit.IN_MODE=0;//epwma source
EPwm1Regs.DBCTL.bit.OUT_MODE = DB_FULL_ENABLE; // =3：双边延时
EPwm1Regs.DBCTL.bit.POLSEL = DB_ACTV_LOC; // =1：ePWMA翻转，ePWMB不翻转
EPwm1Regs.DBFED= deadtime; // FED = 200 TBCLKs
EPwm1Regs.DBRED= deadtime; // RED = 200 TBCLKs

EPwm1Regs.ETSEL.bit.INTSEL=1;//下溢中断：TBCTR=0时，产生中断
EPwm1Regs.ETSEL.bit.INTEN=1;//使能EPWM1_INT中断
EPwm1Regs.ETPS.bit.INTPRD = ET_1ST; // 在第一个事件产生时，产生中断
...
EALLOW;
    SysCtrlRegs.PCLKCR0.bit.TBCLKSYNC = 1;  //使能PWM模块时基时钟同步
    EDIS;
...
// Comment out other unwanted lines.
   ...
   GpioCtrlRegs.GPAQSEL1.bit.GPIO6 = 0;// Synch to SYSCLKOUT GPIO6 (EPWMSYNCI)与系统时钟同步
